...
@implementation PSCloudVideoViewController
    ...
    if ([request isKindOfClass:[LEOWebDAVPropertyRequest class]]) {
        for (LEOWebDAVItem *item in result) {
            if (item.type == LEOWebDAVItemTypeCollection) {
                ...
                if ([item.displayName hasSuffix:@"avi"]){
...
{
    NSString *localFilePath = [NSString stringWithFormat:@"%@/%@",[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0],kVIDEODIRECTORY];
    ...
    if (self.arrgmentType == ArragmentTypeList) {
        ...
        if (_segmentedControl.selectedSegmentIndex==0)
        {
            ...
            cell.fileImageView.image = [UIImage imageNamed:@"file_video.png"];
        }else
        {
